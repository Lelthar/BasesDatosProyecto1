 ...
 * @author mailon2
 */
public class VentanaNaturalJoin extends javax.swing.JFrame {
    
    OperacionesAlgebraRelacional ventanaMain;
    /**
     * Creates new form VentanaProductoCartesiano
     */
    public VentanaNaturalJoin() {
        initComponents();
    }
    
    public VentanaNaturalJoin(OperacionesAlgebraRelacional ventana){
        initComponents();
        this.ventanaMain = ventana;
        this.setTitle("Realización de Concatenación natural");
                    ...
                    if(Singleton.getInstance().getConexionServidor().existeTablaP(jTextFieldTabla2.getText())){
                        ...
                        (Singleton.getInstance().getConexionServidor().esTablaTemporalP(jTextFieldResul.getText()) != true)){
                            try {
                                ...
                                ArrayList<String> comunes = Singleton.getInstance().getConexionServidor().atributosComunes(jTextFieldTabla1.getText(), jTextFieldTabla2.getText());
                               
                                if(comunes.size()>=2){
                                    String cons = "SELECT ";
                                    String consA = "SELECT ";
                                    ArrayList<String> tabla1 = Singleton.getInstance().getConexionServidor().obtenerAtributos(jTextFieldTabla1.getText());

                                    for(int t=0;t<tabla1.size();t++){
                                        cons+=nombreTabla1+"."+tabla1.get(t);
                                        consA+=jTextFieldTabla1.getText()+"."+tabla1.get(t);
                                        if(t!=tabla1.size()-1){
                                            cons+=", ";
                                            consA+=", ";
                                        }
                                    }
                                    ArrayList<String> tabla2 = Singleton.getInstance().getConexionServidor().atributosCorrectos(jTextFieldTabla2.getText(),comunes);
                                    for(int k=0;k<tabla2.size();k++){
                                         cons+=", "+nombreTabla2+"."+tabla2.get(k);
                                         consA+=", "+jTextFieldTabla2.getText()+"."+tabla2.get(k);
                                    }
                                    cons+=" INTO #"+jTextFieldResul.getText();
                                    cons+= " FROM "+nombreTabla1+" , "+nombreTabla2+" WHERE ";
                                    consA+= " FROM "+jTextFieldTabla1.getText()+" , "+jTextFieldTabla2.getText()+" WHERE ";
                                    for(int i=0;i<comunes.size();i++){
                                        cons+= nombreTabla1+"."+comunes.get(i)+" = "+nombreTabla2+"."+comunes.get(i)+" ";
                                        consA+= jTextFieldTabla1.getText()+"."+comunes.get(i)+" = "+jTextFieldTabla2.getText()+"."+comunes.get(i)+" ";
                                        if(i!=comunes.size()-1){
                                            cons+= " AND ";
                                            consA+= " AND ";
                                        }
                                    ...
                                    Singleton.getInstance().getConexionServidor().realizarInstruccionSql(1,cons);
                    ...
                    if(Singleton.getInstance().getConexionServidor().existeTablaP(jTextFieldTabla2.getText())){
                        try {
                            ...
                            if(comunes.size()>=2){
                                ...
                                this.jTextAreaSql.setText(consA);
                                this.jTextAreaAlg.setText(algebra);
                                Singleton.getInstance().getConexionServidor().realizarInstruccionSql(0,cons);
                                Singleton.getInstance().getAdministrador().tablaNaturalJoin(this);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(VentanaNaturalJoin.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new VentanaNaturalJoin().setVisible(true);
            }
        });
    }
